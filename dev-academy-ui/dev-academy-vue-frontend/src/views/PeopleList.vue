    ...
    <table class="table is-stripped is-fullwidth">
      ...
      <tbody>
        ...
        <tr v-for="person in people" :key="person.id">
          ...
          <td :class="[ person.palindrome ? 'text-green' : 'text-red' ]" >{{(person | palindrome) ? 'Yes' : 'No'}}</td>
          <td :class="[ person.authorised ? 'text-green' : 'text-red' ]" >{{person.authorised ? 'Yes' : 'No'}}</td>
          <td :class="[ person.enabled ? 'text-green' : 'text-red' ]" >{{person.enabled ? 'Yes' : 'No'}}</td>
...
export default Vue.extend({
  ...
  filters: {
    colourNames: (colours: IColour[]): string => {
      return colours.map(function(colour){return colour.name}).sort().join(", ");
      // TODO: Step 4
      //
    ...
    palindrome: (person: IPerson): boolean => {
      ...
      var name1 = fullName.replace(" ","").toLowerCase().split("").toString();
      var name2 = fullName.replace(" ","").toLowerCase().split("").reverse.toString();
